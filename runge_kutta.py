def runge_kutta_2nd_order(function: callable, first_value: tuple[float, float], iterations: int, domain: tuple[float, float]) -> list[tuple[float, float]]:
    """
    ...
    for i in range(iterations):
        ...
        yi = y_values[i][1] + .5*(k1i + k2i)
        y_values.append([xi+h,yi])
        xi += h
    
    return y_values

def runge_kutta_4nd_order(function: callable, first_value: tuple[float, float], iterations: int, domain: tuple[float, float]) -> list[tuple[float, float]]:
    """
    Approximates the solution of a first-order ordinary differential equation (ODE)
